...
import { useInfiniteProducts } from '@/app/hooks/useProduct'
...
function EmptyState({ hasFilters, search }: { hasFilters: boolean; search?: string }) {
  return (
    <div className="flex flex-col items-center gap-4 py-20 text-center animate-fade-in animate-duration-500">
      {/* Simple illustration */}
...
const GRID =
  'grid grid-cols-2 gap-x-5 gap-y-8 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6'
      ...
      <div className={GRID}>
        {products.map((product) => (
          <Link key={product.id} href={`/productos/${product.slug}`}>
            <ProductCard product={product} isOwn={product.user.id === me?.id} />
          </Link>
        ))}
        {isFetchingNextPage &&
