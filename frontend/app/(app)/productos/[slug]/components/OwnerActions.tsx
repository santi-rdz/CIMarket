...
export default function OwnerActions({ product }: Props) {
  ...
  const { mutate: changeStatus, isPending: statusPending } = useUpdateProductStatus(product.id)
    ...
    <>
      ...
      <Modal.Content name="delete-product" width="md" height="auto">
        <DeleteConfirm
          ...
          onConfirm={() => remove(undefined, { onSuccess: () => router.push('/perfil/publicaciones') })}
        />
      </Modal.Content>
    </>
  )
...
function DeleteConfirm({ productTitle, isPending, onConfirm, onCloseModal }: {
  productTitle: string
  isPending: boolean
  onConfirm: () => void
  onCloseModal?: () => void
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <p className="txt-6 leading-relaxed text-slate-400">
            Se eliminará{' '}
            <span className="font-semibold text-slate-700">"{productTitle}"</span>{' '}
